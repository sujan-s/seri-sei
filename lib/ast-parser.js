...
const extractImports = (code, config) => {
    try {
        ...
        if (firstImportLine !== -1 && lastImportLine !== -1) {
            ...
            // Additional pass: scan the entire region around imports for any scattered generated headers
            // This catches cases where duplicate headers exist outside the main block
            const scanStart = Math.max(0, blockStart - 10);
            const scanEnd = Math.min(lines.length, blockEnd + 10);
