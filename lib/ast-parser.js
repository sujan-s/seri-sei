...
// Ignore comment patterns - supports both seri-sei-ignore and seri-seiyaadhe
const IGNORE_PATTERNS = {
    nextBlock : /^\s*\/\/\s*(seri-sei-ignore|seri-seiyaadhe)\s*$/,
    start     : /^\s*\/\/\s*(seri-sei-ignore-start|seri-seiyaadhe-start)\s*$/,
    end       : /^\s*\/\/\s*(seri-sei-ignore-end|seri-seiyaadhe-end)\s*$/,
};

/**
 * Finds all ignored ranges in the code (between start/end comments).
 * @param {string[]} lines - The source code lines
 * @returns {Array<{start: number, end: number}>} Array of ignored line ranges (0-based)
 */
const findIgnoredRanges = (lines) => {
    const ranges = [];
    let currentStart = -1;

    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        if (IGNORE_PATTERNS.start.test(line)) {
            currentStart = i;
        } else if (IGNORE_PATTERNS.end.test(line) && currentStart !== -1) {
...
/**
 * Checks if a line number falls within any ignored range.
 * @param {number} lineNum - The line number to check (0-based)
 * @param {Array<{start: number, end: number}>} ignoredRanges - The ignored ranges
 * @returns {boolean}
 */
const isInIgnoredRange = (lineNum, ignoredRanges) => {
    return ignoredRanges.some(range => lineNum >= range.start && lineNum <= range.end);
};

/**
 * Checks if a node has an ignore comment on the preceding line.
 * @param {number} nodeStartLine - The node's start line (0-based)
 * @param {string[]} lines - The source code lines
 * @returns {boolean}
 */
const hasIgnoreComment = (nodeStartLine, lines) => {
    if (nodeStartLine <= 0) return false;
    const precedingLine = lines[nodeStartLine - 1];
    return IGNORE_PATTERNS.nextBlock.test(precedingLine);
};
...
const extractImports = (code, config) => {
    try {
        ...
        const ignoredRanges = findIgnoredRanges(lines);
        ...
        traverse(ast, {
            ImportDeclaration(path) {
                ...
                if (isInIgnoredRange(importStartLine, ignoredRanges) ||
                    hasIgnoreComment(importStartLine, lines)) {
                    return;
                }
    ...
    try {
        ...
        traverse(ast, {
            TSInterfaceDeclaration(path) {
                ...
                // Skip if this block should be ignored
                if (isInIgnoredRange(startLine, ignoredRanges) ||
                    hasIgnoreComment(startLine, lines)) {
                    return;
                }
