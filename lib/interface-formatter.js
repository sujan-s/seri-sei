...
const formatInterfaceBlocks = (interfaceBlocks, originalLines, config) => {
    ...
    for (const block of interfaceBlocks) {
        ...
        // Find the opening brace position of the interface/type body
        // Must track angle bracket depth to avoid matching braces inside generic constraints
        // e.g., in "interface Foo<T extends {}> {}", the first {} is a constraint, not the body
        ...
        for (let i = 0; i < blockLines.length; i++) {
            ...
            for (let j = 0; j < line.length; j++) {
                ...
                if (char === "<") {
                    ...
                    // Found opening brace outside of angle brackets - this is the body
                    braceLineIndex = i;
                    braceColumnIndex = j;
                    break outerLoop;
                }
        ...
        // But skip if the opening brace is on the same line (content between braces already handled above)
        if (blockLines.length > 1 && braceLineIndex < blockLines.length - 1) {
        ...
        // If the body is empty, keep {} on the same line
...
const formatBlockContent = (blockContentLines, baseIndent, config = {}) => {
    ...
    for (const line of expandedLines) {
        ...
        // Comments should be treated as separate entries so they don't consume the next property
        if (trimmedLine.startsWith("//") || trimmedLine.startsWith("/*")) {
            // If we have pending property lines, push them first
            if (currentPropLines.length > 0) {
                properties.push(currentPropLines);
                currentPropLines = [];
                braceDepth = 0;
                parenDepth = 0;
            }
            // Push the comment as its own entry
...
const formatUnionType = (blockLines, baseIndent, config) => {
    ...
    for (let i = equalsLineIndex + 1; i < blockLines.length; i++) {
        ...
        if (trimmed.startsWith("|")) {
            ...
            // Check if it's a single-line object type (not a template literal)
            // Template literals use ${...} for interpolation, so exclude those
            const isTemplateLiteral = content.startsWith("`");
            if (!isTemplateLiteral && content.includes("{") && content.includes("}")) {
